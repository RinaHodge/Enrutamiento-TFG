...
def generar_dataset():
    ...
    Nodo TA: nodo con menor EPDD en la ruta 
    """
    nombre_archivo = 'Abilene_v3_bidireccional.csv'
    archivo_existe = os.path.exists(nombre_archivo)

    #Crear el archivo CSV
    with (open(nombre_archivo, mode='a', newline='') as file_csv
        #  ,        open('Consola_v2.txt', mode='w', encoding='utf-8') as file_txt
        ):
        ...
        num_aristas = grafo.get_num_aristas()     #Número total de aristas

        if not archivo_existe:
            cabecera = ['Topologia'] + ['id'] * total_nodos + ['Prob_perdida'] * num_aristas + ['Delay'] * num_aristas + ['Nodo_TA']
        ...
        for tm in ['TM1', 'TM2', 'TM3', 'TM4', 'TM5']:
            ...
            rutas, exito = enrutar(grafo, trafico, k=36, tipo_enrutamiento=1)
            ...
            #        print(f"    Delay: {'1ms' if delay == 1 else 'aleatorio entre 1ms y 10ms'}")
                    ...
                    #if delay == 1 and probabilidad == 1:
                    #    vueltas = 1
                    #else: 
                    #    vueltas = 250
                    #for iteracion in range(vueltas):
            while casos_actuales < casos_por_tm:
                valores_prob = []
                valores_delay = []
                        #el for debeñia empezar justo aqui
                for origen in grafo.grafo:
                    for destino in grafo.grafo[origen]:
                        ...
                        prob_grafo = round(random.uniform(0.0, 0.1), 2)
                        delay_grafo = round(random.uniform(1, 10), 2)
                        
                        grafo.set_probabilidad_perdida(origen, destino, prob_grafo)
                        grafo.set_delay(origen, destino, delay_grafo)

                        valores_prob.append(prob_grafo)
                        valores_delay.append(delay_grafo)
                ...
                if exito:
                    ...
                    if nodo_TA_optimo_global == 'S5':
                        ...
                        fila_datos = [1] + valores_trafico + valores_prob + valores_delay + [nodo_TA_optimo_global]   # 1 porque es Abilene
                        writer.writerow(fila_datos)
                        print(f"Por la matriz {tm}")
